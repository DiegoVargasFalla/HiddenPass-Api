...
public class PasswordServiceImpl implements PasswordService {
     ...
     * @param passwordEntityDTO object to add password
     * @return object password
     */
    @Override
    @Transactional
    public Optional<PassWordEntity> createPassWord(String email, PasswordEntityDTO passwordEntityDTO) throws Exception{
        ...
        if (userExisting.isPresent()) {
            ...
            passWordDTO.getPassWord().setPassword(encryptedPassword);

            passWordDTO.getPassWord().setUser(user);

            PassWordEntity passwordSaved =  passwordRepository.save(passWordDTO.getPassWord());
    ...
    @Override
    @Transactional
    public ArrayList<PasswordResponseDTO> getPassWordsFromUser(String emailUser, MasterKeyDTO masterKeyDTO) throws Exception{
        ...
        if (userExisting.isPresent()) {
            ...
            for (PassWordEntity password: user.getListPass()) {
                try {
                    ...
                    passwordResponseDTO.setPassword(passwordEncryptedWithAES);
    ...
    @Override
    @Transactional
    public Optional<PasswordResponseDTO> updatePassWord(String emailUser, PasswordEntityDTO passwordEntityDTO) throws Exception{
        ...
        if (existingUser.isPresent()) {
            ...
            Optional<PassWordEntity> ExistingPassWordEntity =  passwordRepository.findById(decryptedPasswordEntityDTO.getPassWord().getId());
            ...
            if (ExistingPassWordEntity.isPresent()) {
                ...
                if (passWordEntity.getId().equals(decryptedPasswordEntityDTO.getPassWord().getId())) {

                    if (decryptedPasswordEntityDTO.getPassWord().getUsername() != null) {
                        passWordEntity.setUsername(decryptedPasswordEntityDTO.getPassWord().getUsername());
                    }
                    if (decryptedPasswordEntityDTO.getPassWord().getPassword() != null) {
                        if (checkPasswordOrUrlUniqueFromUser(decryptedPasswordEntityDTO.getPassWord().getPassword(), user, "P", decryptedPasswordEntityDTO.getMasterKey())) {
                            ...
                            passWordEntity.setPassword(encrypt);
                        } else {
                            throw new IllegalArgumentException("Password already exists");
                        }
                    }
                    if (decryptedPasswordEntityDTO.getPassWord().getUrl() != null) {
                        if(checkPasswordOrUrlUniqueFromUser(decryptedPasswordEntityDTO.getPassWord().getUrl(), user, "U", decryptedPasswordEntityDTO.getMasterKey())) {
                            passWordEntity.setUrl(decryptedPasswordEntityDTO.getPassWord().getUrl());
                        } else {
                            throw new IllegalArgumentException("Url already exists");
                        }
                    }
                    if (decryptedPasswordEntityDTO.getPassWord().getNote() != null) {
                        passWordEntity.setNote(decryptedPasswordEntityDTO.getPassWord().getNote());
                    }

                    passwordRepository.save(passWordEntity);
                    return Optional.of(passwordResponseDTO);
                }
            }
        }
