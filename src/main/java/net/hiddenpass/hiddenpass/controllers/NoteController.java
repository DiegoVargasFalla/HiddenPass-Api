...
public class NoteController {
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'USER')")
    public ResponseEntity<ArrayList<NoteEntity>> getUserNotes(@RequestHeader("Authorization") String token, @Valid @RequestBody MasterKeyDTO masterKeyDTO) throws Exception{
        String email = userService.getUsernameFromToken(token);

        if (email == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        }
        return ResponseEntity.status(HttpStatus.OK).body(noteService.getAllNotes(email, masterKeyDTO));
    }

    @PatchMapping("/update-note")
    @PreAuthorize("hasAnyRole('ADMIN', 'USER')")
    public ResponseEntity<?> updateNote(@RequestHeader("Authorization") String token, @Valid @RequestBody  NoteEntityUpdateDTO noteEntityUpdateDTO) throws Exception {
        String email = userService.getUsernameFromToken(token);

        if (email == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        }
        return ResponseEntity.status(HttpStatus.OK).body(noteService.updateNote(email, noteEntityUpdateDTO));
    }
